...
public class UsuarioDAO {
    ...
    public java.util.ArrayList<modelo.Usuario> listarUsuariosPorRol(String rolBuscado) {
        ...
        try (java.sql.Connection con = conexion.ConexionBD.conectar(); java.sql.PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    public List<Usuario> listarUsuarios() {
        List<Usuario> lista = new ArrayList<>();
        String sql = "SELECT id_usuario, nombre, apellido FROM usuario";
        try (Connection con = ConexionBD.conectar(); PreparedStatement ps = con.prepareStatement(sql); ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                Usuario u = new Usuario();

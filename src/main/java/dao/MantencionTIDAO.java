...
public class MantencionTIDAO {
    ...
    PreparedStatement.RETURN_GENERATED_KEYS
            ...
            // fecha_termino (puede ser null si quedó pendiente)
...
if (keys.next()) {
    idMantencion = keys.getInt(1);
            ...
            if (con != null) try { con.rollback(); } catch (SQLException ex) { System.err.println(ex); }
    ...
    public int registrarMantencionCorrectiva(MantencionTI mant, List<Integer> idsRepuestos) {
        ...
        try {
            ...
            psMant.setString(3, mant.getDescripcionFalla());
            psMant.setString(4, mant.getAccionRealizada());

            psMant.setTimestamp(5, new Timestamp(
                    mant.getFechaInicio() != null ? mant.getFechaInicio().getTime() : System.currentTimeMillis()));

            if (mant.getFechaTermino() != null)
                psMant.setTimestamp(6, new Timestamp(mant.getFechaTermino().getTime()));
            else
                psMant.setNull(6, Types.TIMESTAMP);

            psMant.setString(7, mant.getEstadoEquipo() != null ? mant.getEstadoEquipo() : "OPERATIVO");
            psMant.executeUpdate();

            // Obtener ID generado para uso_repuestos_ti
            int idMantencion = -1;
            try (ResultSet keys = psMant.getGeneratedKeys()) {
                if (keys.next()) idMantencion = keys.getInt(1);
            }

            if (idMantencion > 0 && idsRepuestos != null && !idsRepuestos.isEmpty()) {
                PreparedStatement psUso   = con.prepareStatement(sqlUso);
        ...
        try (Connection con = ConexionBD.conectar();
            ...
            while (rs.next()) {
                lista.add(new Object[]{
                    ...
                    rs.getTimestamp("fecha_termino"),   // puede ser null
    ...
    //  DETALLE DE UNA MANTENCIÓN (para ventana de detalle)
    ...
    /**
     * Actualiza únicamente el estado del equipo de una mantención.
     */

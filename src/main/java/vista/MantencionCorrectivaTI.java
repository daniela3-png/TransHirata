...
public class MantencionCorrectivaTI extends javax.swing.JFrame {
    ...
    public MantencionCorrectivaTI() {
        ...
        DefaultTableModel nuevoModelo = new DefaultTableModel(
                new Object[][]{},
                new String[]{"ID", "Repuesto", "Stock Disponible", "Solicitar"}
        ) {
        ...
        jTableRepuestos.setModel(nuevoModelo);
...
setLocationRelativeTo(null);
    ...
    private void cargarRepuestos() {
        ...
        for (modelo.Repuesto r : lista) {
            modeloTabla.addRow(new Object[]{
                Integer.valueOf(r.getIdRepuesto()),
                r.getNombrePieza(),
                Integer.valueOf(r.getStockActual()),
                Boolean.FALSE
            });
        }
    }
    ...
    private void cargarRepuestosPorFiltro(String equipoSel) {
        ...
        String cat = "";
        if (equipoUpper.contains("NOTEBOOK")) {
            cat = "NOTEBOOK";
        } else if (equipoUpper.contains("CPU") || equipoUpper.contains("PC")) {
            cat = "PC";
        } else if (equipoUpper.contains("PRN") || equipoUpper.contains("IMPRESORA")) {
            cat = "IMPRESORA";
        }
    ...
    private void filtrarEquiposPorUsuario() {
        ...
        if (cbEquipos.getItemCount() > 0 && !cbEquipos.getSelectedItem().toString().equals("Sin equipos asignados")) {
    ...
    private void btnGuardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGuardarActionPerformed
        if (cbEquipos.getSelectedIndex() == -1 || cbEquipos.getSelectedItem().toString().equals("Sin equipos asignados")) {
        ...
        if (falla.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Ingrese la falla detectada.");
            return;
        }
        ...
        for (int i = 0; i < jTableRepuestos.getRowCount(); i++) {
            ...
            int stockActual = (int) jTableRepuestos.getValueAt(i, 2);
            Boolean sol = (Boolean) jTableRepuestos.getValueAt(i, 3);

            if (sol != null && sol) {
                if (stockActual <= 0) {
                    JOptionPane.showMessageDialog(this, "No queda stock de: " + nombrePieza, "Stock insuficiente", JOptionPane.ERROR_MESSAGE);
                    return;
                }
                seleccionados.add(idRep);
            }
        ...
        String folio = String.format("MT-%s-%04d", anio, idMantencion);

        JFileChooser fc = new JFileChooser();
        fc.setDialogTitle("Guardar formulario PDF correctivo");
        fc.setSelectedFile(new java.io.File("Correctivo_RF06_" + new SimpleDateFormat("yyyyMMdd_HHmm").format(new Date()) + ".pdf"));
        ...
        String usuarioEquipo = (listaUsuarios != null && cbUsuarios.getSelectedIndex() >= 0) ? cbUsuarios.getSelectedItem().toString() : "";
        String tipoFalla = rbHardware.isSelected() ? "Hardware" : rbSoftware.isSelected() ? "Software" : "No especificado";
        String prioridad = rbAlta.isSelected() ? "Alta" : rbMedia.isSelected() ? "Media" : rbBaja.isSelected() ? "Baja" : "No especificado";
        ...
        String[][] piezasArr = piezasParaPDF.isEmpty() ? new String[][]{{"", "", ""}, {"", "", ""}} : piezasParaPDF.toArray(new String[0][]);

        try {
            GeneradorPDFMantencionTI.generarCorrectivo(
                    idMantencion, folio, String.valueOf(equipo.getIdActivo()),
                    new SimpleDateFormat("dd/MM/yyyy").format(new Date()), txtHoraInicio.getText().trim(),
                    ...
                    falla, tipoFalla, prioridad, accionesArr, piezasArr, ruta
            );
...
private void cbEquiposActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_cbEquiposActionPerformed
    if (cbEquipos.getSelectedItem() != null && !cbEquipos.getSelectedItem().toString().equals("Sin equipos asignados")) {
        cargarRepuestosPorFiltro(cbEquipos.getSelectedItem().toString());
    }
